...
import it.unibo.dcs.service.webapp.repositories.datastores.api.exceptions.CreateRoomException
...
class RoomRestApi(private[this] val discovery: HttpEndpointDiscovery)
  ...
  override def createRoom(createRoomRequest: CreateRoomRequest): Observable[Room] = {
    ...
    request((roomWebClient: WebClient) =>
      ...
      .map(response => response.bodyAsJsonObject().getOrElse(throw CreateRoomException()))
    */
    for {
      response <- request((roomWebClient: WebClient) =>
        Observable.from(roomWebClient.post("/api/room/").sendJsonObjectFuture(createRoomRequest)))
    } yield response.bodyAsJsonObject().getOrElse(throw CreateRoomException())
  }
}
